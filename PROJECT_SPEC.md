...
## Gate observability pilot (2026-09-23)

The standalone gate model and offline runner provide bounded paired sessions:
normal operation, command delivery delay, opening stall and open-contact failure.
Observations, validated command/result payloads, ground truth and run provenance
are saved separately under ignored experiment outputs. The illustrative model
has time-dependent motion and optional current, not calibrated servo physics.
